...
export async function POST(request: Request): Promise<NextResponse> {
  return withRateLimit(request, rateLimiters.chat, async (req) => {
    ...
    // Modified section: Use generateContent instead of generateContentStream
    ...
    if (attemptMultimodal) {
      ...
      if (pdfFetchSuccess) {
          ...
          try {
            ...
            console.log("Multimodal generation completed successfully.");
          } catch (multiModalError: any) {
            console.error('Error during multimodal generation:', multiModalError);
            contentResult = null; // Ensure fallback happens
            generationMode = 'multimodal-fallback';
            console.log("Falling back to text-only generation due to multimodal error.");
          }
      } else {
    ...
    if (!contentResult) {
      ...
      try {
        ...
        console.log(`${generationMode} generation completed successfully.`);
      } catch (textOnlyError: any) {
        console.error(`Error during ${generationMode} generation:`, textOnlyError);
        return NextResponse.json(
          { error: `Failed to generate content: ${textOnlyError.message || 'Unknown Gemini error'}` },
          { status: 500 }
        );
    ...
    if (!contentResult) {
        console.error("Error: No content result available after generation attempts.");
        return NextResponse.json(
    ...
    try {
        // Extract the text from the content result
        const generatedText = contentResult.response.text();
